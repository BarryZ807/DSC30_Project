...
/**
 * A run time analysis class of sorting
 * @author Zehui Zhang
 * @since  2021-02-05
 */
public class RuntimeAnalysis {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < NUM_TEST; i++) {
            ArrayList<Integer> data = randomNumbers(numData, MIN, MAX);
            ArrayList<Integer> data1 = deepCopyArrayList(data);
            ArrayList<Integer> data2 = deepCopyArrayList(data);
            timeInsertionSort(data, NUM_RUN);
            timeMergeSort(data1, NUM_RUN);
            timeQuickSort(data2, NUM_RUN);
            numData += numData;
        }
        
        numData = NUM_RUN * MAX;
        for (int i = 0; i < QuickSortCutoffValues.length; i++) {
            ArrayList<Integer> data = randomNumbers(numData, MIN, MAX);
            timeModifiedQuickSort(data, NUM_RUN, QuickSortCutoffValues[i]);
        }
